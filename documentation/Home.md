---
layout: default
title: Home.md
---

# Home.md


### [☰](/documentation/setup) 1. Setup.md
0. [Install Haxe](/documentation/setup/install-haxe)
0. [Install Lime](/documentation/setup/install-lime)
0. [Install OpenFL](/documentation/setup/install-openfl)
0. [Platforms](/documentation/setup/platforms)
    0. [Setup Windows](/documentation/setup/platforms/setup-windows)
    0. [Setup Mac](/documentation/setup/platforms/setup-mac)
    0. [Setup Linux](/documentation/setup/platforms/setup-linux)
    0. [Setup iOS](/documentation/setup/platforms/setup-ios)
    0. [Setup Android](/documentation/setup/platforms/setup-android)
    0. [Setup BlackBerry](/documentation/setup/platforms/setup-blackberry)
    0. [Setup Tizen](/documentation/setup/platforms/setup-tizen)
    0. [Setup Flash](/documentation/setup/platforms/setup-flash)
    0. [Setup HTML5](/documentation/setup/platforms/setup-html5)

### [☰](/documentation/projects) 2. Projects.md
0. [Create New Project](/documentation/projects/create-new-project)
0. [Samples](/documentation/projects/samples)
0. [Project Files](/documentation/projects/project-files)
    0. [XML Format](/documentation/projects/project-files/xml-format)
    0. [HXP Format](/documentation/projects/project-files/hxp-format)

### [☰](/documentation/command-line-tools) 3. Command-Line-Tools.md
0. [Basic Commands](/documentation/command-line-tools/basic-commands)

### [☰](/documentation/basics) 4. Basics.md
0. [Bitmaps](/documentation/basics/bitmaps)
    0. [Load BitmapData](/documentation/basics/bitmaps/load-bitmapdata)
    0. [Display Images](/documentation/basics/bitmaps/display-images)
    0. [Change BitmapData](/documentation/basics/bitmaps/change-bitmapdata)
    0. [Unload BitmapData](/documentation/basics/bitmaps/unload-bitmapdata)
0. [Animation](/documentation/basics/animation)
    0. [Actuate](/documentation/basics/animation/actuate)
    0. [Enter Frame](/documentation/basics/animation/enter-frame)
    0. [Timers](/documentation/basics/animation/timers)
0. [Events](/documentation/basics/events)
    0. [Mouse Events](/documentation/basics/events/mouse-events)
