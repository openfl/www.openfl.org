...
Game.prototype = {
	...
	,updateUI: function() {
		if(this.uiBar == null) {
			...
			this.uiBar.set_scaleX(this.uiBar.set_scaleY(2));
		}
		var border = 15790320;
		var bg = 6316128;
	...
	,win: function() {
		...
		this.mask.set_x(this.output.width / 2);
		this.mask.set_y(this.output.height / 2);
	...
	,gameOver: function() {
		...
		mc.set_y(Std["int"]((this.output.height - mc.get_height()) * 0.5));
		this.root.addChild(mc);
	}
	,updateWeb: function() {
	...
	,update: function() {
		...
		this.scroll.mc.set_x(-sx);
		this.scroll.mc.set_y(-sy);
		this.scroll.mc.set_scaleX(this.scroll.mc.set_scaleY(z));
		this.hero.update(dt);
		Popup.updateAll(dt);
		Part.updateAll(dt);
		var tap = false;
		...
		this.pixelFilterBMP.set_alpha(this.curColor.alpha);
...
Entity.prototype = {
	...
	,explode: function(p) {
		if(p == null) p = 100;
		Part.explode(this.bmp.bitmapData,Std["int"](this.mc.get_x()),Std["int"](this.mc.get_y()),p);
	}
	...
	,update: function(dt) {
		this.updatePos(dt);
		this.mc.set_x(this.x * 16 | 0);
		this.mc.set_y((this.y * 16 | 0) - 2);
		if(this.shade != null) {
			this.shade.set_x(this.mc.get_x() + 0.5);
			this.shade.set_y(this.mc.get_y() + 0.5);
		}
		if(this.frame >= 0) {
...
Hero.prototype = $extend(Entity.prototype,{
	...
	,update: function(dt) {
		...
		if(this.hitRecover > 0) {
			this.hitRecover -= dt;
			this.mc.set_alpha(Math.abs(Math.sin(this.hitRecover)));
			if(this.hitRecover <= 0) this.mc.set_alpha(1);
		}
	...
	,updateSword: function(dt) {
		...
		this.sword.mc.set_x(this.mc.get_x() + 8 + (this.sword.pos - 1) * this.sword.dx);
		this.sword.mc.set_y(this.mc.get_y() + 8 + (this.sword.pos - 1) * this.sword.dy + (this.sword.dx != 0?2:0));
		var hitX = this.sword.mc.get_x() + this.sword.dx * 10;
		var hitY = this.sword.mc.get_y() + this.sword.dy * 10;
		var hx = hitX / 16 | 0;
		var hy = hitY / 16 | 0;
	...
	,attack: function() {
		...
		bmp.set_y(-3);
		smc.addChild(bmp);
		smc.set_rotation(Math.atan2(-this.dirX,this.dirY) * 180 / Math.PI);
	...
	,update: function(dt) {
		if(this.wait > 0) {
			...
			case 9:
				if(this.frame > 12) {
					var _g11 = this.mc;
					_g11.set_alpha(_g11.get_alpha() - dt * 0.03);
					if(this.mc.get_alpha() <= 0) {
						this.frame = 0;
...
flash_display_DisplayObject.prototype = $extend(flash_events_EventDispatcher.prototype,{
	...
	,__setStageReference: function(stage) {
		if(this.stage != stage) {
			this.stage = stage;
			if(stage != null) this.dispatchEvent(new flash_events_Event(flash_events_Event.ADDED_TO_STAGE,false,false));
		}
	}
	,__update: function() {
		this.__renderable = this.get_visible() && this.get_alpha() > 0 && this.get_scaleX() != 0 && this.get_scaleY() != 0 && !this.__isMask;
...
Part.prototype = {
	update: function(dt) {
		...
		this.mc.set_x(this.x);
		this.mc.set_y(this.y - this.z);
		this.time -= dt;
		this.mc.set_alpha(this.time / 30);
		return this.time > 0;
	}
	,remove: function() {
...
var Title = function(game) {
	...
	this.root.set_scaleX(this.root.set_scaleY(2));
...
Title.prototype = {
	...
	,update: function(_) {
		...
		this.layer2.set_y(100 - d2 * 2);
		this.layer2.set_x(25 - d2 * 0.5);
		this.layer3.set_y(Math.sin(this.time * 0.1) * 10);
		this.cursor.set_x(105 + Math.sin(this.time) * 2);
		this.cursor.set_y(120 + (this.load?20:0));
		var _g2 = 0;
		var _g11 = [69,13,32];
		while(_g2 < _g11.length) {
			var k1 = _g11[_g2];
...
flash_display_Bitmap.prototype = $extend(flash_display_DisplayObjectContainer.prototype,{
	__getBounds: function(rect,matrix) {
		var bounds = new flash_geom_Rectangle(0,0,this.bitmapData.width,this.bitmapData.height);
...
flash_display_Graphics.prototype = {
	...
	,__render: function() {
		if(this.__dirty) {
			...
			this.__positionX = 0;
			this.__positionY = 0;
			if(this.__commands.length == 0 || this.__bounds.width == 0 || this.__bounds.height == 0) {
				this.__canvas = null;
				this.__context = null;
			} else {
...
flash_display_Stage.prototype = $extend(flash_display_Sprite.prototype,{
	...
	,__render: function() {
		...
		this.__broadcast(new flash_events_Event(flash_events_Event.ENTER_FRAME));
	...
	,window_onKey: function(event) {
		...
		var keyLocation = 0;
		if(event.location != null) keyLocation = js_Boot.__cast(event.location , Int); else if(event.keyLocation != null) keyLocation = js_Boot.__cast(event.keyLocation , Int);
		this.dispatchEvent(new flash_events_KeyboardEvent(event.type == "keydown"?flash_events_KeyboardEvent.KEY_DOWN:flash_events_KeyboardEvent.KEY_UP,true,false,event.charCode,keyCode,keyLocation,event.ctrlKey,event.altKey,event.shiftKey));
	}
	...
	,window_onFocus: function(event) {
		var event1 = new flash_events_Event(flash_events_Event.ACTIVATE);
		this.__broadcast(event1);
	}
	,window_onBlur: function(event) {
		var event1 = new flash_events_Event(flash_events_Event.DEACTIVATE);
		this.__broadcast(event1);
	}
...
flash_events_MouseEvent.__create = function(type,event,local,target) {
	...
	if(type == flash_events_MouseEvent.MOUSE_DOWN || type == flash_events_MouseEvent.MOUSE_MOVE) if(event.which != null) __mouseDown = event.which == 1; else if(event.button != null) __mouseDown = event.button == 0; else __mouseDown = false; else if(type == flash_events_MouseEvent.MOUSE_UP) {
		if(event.which != null) {
...
flash_text_TextField.prototype = $extend(flash_display_InteractiveObject.prototype,{
	__getBounds: function(rect,matrix) {
		var bounds = new flash_geom_Rectangle(0,0,this.__width,this.__height);
...
Timer.deltaT = 1;
Timer.frameCount = 0;
flash_events_Event.ACTIVATE = "activate";
